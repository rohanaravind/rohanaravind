# Rohan Aravind

Aerospace Engineering student at SJSU. I build software tools at the intersection of flight and autonomy.

Background in industrial design. Currently focused on GNC, drone systems, and open-source aerospace tooling.

---

## Projects

### [FlightPath AI](https://github.com/rohanaravind/FlightPathAI)
Browser-based mission planning tool for commercial drone operators. Draw a polygon, get a lawnmower grid flight path, export a mission brief PDF. No account required. Built with Next.js 14, TypeScript, and Mapbox GL JS.

→ [Live demo](https://flightpathai.vercel.app)

### [Turnout](https://github.com/rohanaravind/Turnout)
Volunteer hours tracking platform. Built for organizations that still track service hours on paper sign-in sheets. Production app — Next.js 14, TypeScript, Supabase, Vercel.

---

## Background

- JROTC VEX Robotics — competed at the national championship in Sandusky, Ohio. Placed 44th of 94 teams. Built and led the team from scratch in under three months.
- Previously in industrial design — background shows up in how I approach interfaces and system communication.
- Contributing to ArduPilot — open-source autopilot software that flies real drones and aircraft.

---

## Stack

**Languages** — TypeScript, Python, C++ (extending from VEX robotics background)

**Frontend** — Next.js 14, React, Tailwind CSS, Mapbox GL JS, Framer Motion

**Backend / infra** — Supabase, Vercel, Node.js

**Aerospace / hardware** — ArduPilot, SITL simulation, Fusion 360

---

## Writing

I write about building, engineering, and aerospace on Medium.

→ [The Nationals Journey: What I Learned Leading a Team That Didn't Want to Be Led](https://medium.com/@rohanaravind)

---

## Contact

[LinkedIn](https://linkedin.com/in/rohanaravind) · [Medium](https://medium.com/@rohanaravind) · rohan@rohanaravind.com
